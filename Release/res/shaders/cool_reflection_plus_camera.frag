...
void main() {
	...
	vec3 base_color = display_world_vector(normals_raw);
	...
	float spec = pow(clamp(dot(c, reflection), 0, 1), 32);
	float specular_lighting = 1.0 * spec / (distance); 

	vec3 lighting = (ambient_lighting ) * light_color;

	//display_world_vector(normals_raw)
	//lighting * base_color + (specular_lighting * light_color)
	color = vec4(display_world_vector(c * reflection * 3), 1);
