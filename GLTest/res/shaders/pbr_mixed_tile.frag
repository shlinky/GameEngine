...
void main() {
	...
	vec3 nnorm = vec3(texture(normalTex, UVt)) * 2 - 1;
	...
	float rough = vec3(texture(ORM, UVt)).y;
	float metal = vec3(texture(ORM, UVt)).z;
	vec3 base_color = vec3(texture(colorTex, UVt));
